...
contract DappLottery is Ownable(msg.sender) {
    ...
    mapping(uint256 => ParticipantStruct[]) public lotteryParticipants;
    mapping(uint256 => string[]) public lotteryLuckyNumbers;
    mapping(uint256 => mapping(uint256 => bool)) public LuckyNumberUsed;
    mapping(uint256 => lotteryResultStruct) public lotteryResult;
    ...
    function createLottery(
        ...
        require(expirseAt > currentTime(), "expireAt cannot be less than future");
    ...
    // 一个彩票内有多个幸运号码 可以把彩票看成一个系列 luckyNumberId就是号码ID,第一个id为彩票id第二个为幸运号码id
    function buyTicket(uint256 id, uint256 luckyNumberId) public payable {
        ...
        //添加彩票 参加者的信息
        lotteryParticipants[id].push(ParticipantStruct(msg.sender, lotteryLuckyNumbers[id][luckyNumberId], false));
    ...
    function randomlySelectWinners(uint256 id, uint256 numOfWinners) public {
        ...
        console.log("completed:", lotteryResult[id].completed);
    ...
    function getLotteries() public view returns (LotteryStruct[] memory Lotteries) {
        ...
        for (uint256 i = 0; i < _totalLottrties.current(); i++) {
    ...
    function getLuckyNumberUsed(uint256 lotteryId, uint256 luckyNumberId) public view returns (bool) {
        return LuckyNumberUsed[lotteryId][luckyNumberId];
    }
}
